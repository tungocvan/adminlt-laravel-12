        ...
        <div class="col-md-12">
            <div class="card-header bg-primary d-flex justify-content-between align-items-center text-white">
                ...
                <div class="col-md-3">
                    ...
                    placeholder="Tìm kiếm..." style="float: left;width:90%;margin-right:-35px">

                     <div class="input-group-append" style="height: 38px">
                        @if (!$search)
                            <span style="line-height:38px;color:black"><i class="fas fa-search"></i></span>
                        @else
                            <span wire:click="$set('search', '')" style="line-height:38px;cursor:pointer;color:red"><i
                ...
                <div class="col-md-7 d-flex justify-content-end">
                    <button wire:click="openModal" class="btn btn-light btn-sm mr-1">
                        <i class="fa fa-plus mr-1"></i> Thêm mới
                    </button>
                    <button
                            onclick="if(confirm('Bạn có chắc muốn xóa các user đã chọn?')) { @this.deleteSelected() }"
    ...
    <div class="row my-2">
        <div class="col-md-12 d-flex justify-content-end">
            ...
            <div class="text-right">
                <button wire:click="printList" class="btn btn-outline-secondary btn-sm" title="In danh sách">
    ...
    <div class="table-responsive">
        <table class="table-hover table-bordered mb-0 table">
            <thead class="thead-light">
                <tr>
                    <th><input type="checkbox" wire:model.live="selectAll" wire:click="toggleSelectAll">
                    </th>
                    <th wire:click="sortBy('id')" style="cursor:pointer;width:55px">ID <i class="fas fa-sort text-muted"></i></th>
                    <th wire:click="sortBy('name')" style="cursor:pointer;">Tên <i class="fas fa-sort text-muted"></i>
                    </th>
                    <th wire:click="sortBy('email')" style="cursor:pointer;">Email <i
                            class="fas fa-sort text-muted"></i>
                    </th>
                    <th>Role</th>
                    <th>Xác thực</th>
            ...
            <tbody>
                @forelse($users as $user)
                    <tr>
                        <td style="width:20px"><input type="checkbox" wire:model.live="selected" value="{{ $user->id }}">
                        </td>
                        <td>{{ $user->id }}</td>
                        <td>{{ $user->name }}</td>
                        <td>{{ $user->email }}</td>
                        ...
                        <td class="text-center">
                            <div class="btn-group">
                                <button wire:click="edit({{ $user->id }})" class="btn btn-outline-primary btn-sm"><i
