<div class="p-3">
    ...
    <table id="header_lich" class="table-bordered mb-3 table text-center"></table>
        ...
        <tbody>
            @foreach ($calendar as $week)
                <tr>
                    @foreach ($week as $d)
                        @if ($d)
                            <td onclick="@this.call('selectDay', {{ $d }});" id="d{{ $d }}"
    ...
    <script>
        ...
        function fillAm() {
            for (let d = 1; d <= 31; d++) {
                let el = document.getElementById("al" + d);
                if (!el) continue;
                try {
                    let am = new LunarDate(d, {{ $month }}, {{ $year }});
                    el.innerText = (am.day + "/" + am.month);
                } catch {}
            }
        }
        ...
        function renderHeader(d, m, y) {
            try {
                let L = new LunarDate(d, m, y),
                    jd = jdn(d, m, y);
                const TUAN = ['CN', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7'];
                const TH = ['Một', 'Hai', 'Ba', 'Bốn', 'Năm', 'Sáu', 'Bảy', 'Tám', 'Chín', 'Mười', 'Mười một', 'Mười hai'];
                const TIET = ['Xuân phân', 'Thanh minh', 'Cốc vũ', 'Lập hạ', 'Tiểu mãn', 'Mang chủng', 'Hạ chí', 'Tiểu thử',
                ...
                let cM = CanChi((y - 1900) * 12 + (m - 1) + 12);
                if (d >= L.t) cM = CanChi((y - 1900) * 12 + (m - 1) + 13);
                let cD = CanChi(L.day);
